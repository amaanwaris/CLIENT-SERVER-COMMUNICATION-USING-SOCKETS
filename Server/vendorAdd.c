...
int main(int argc, char *argv[])
{
	...
	if(sock == -1)
	{
		perror("vendorAdd: socket line no. 17");
		exit(EXIT_FAILURE);
	}
	...
	printf("vendorAdd Attempting to connect to port %d ..\n",port);
// Connect back to the specific thread spawned this vendor
	if(connect(sock, (struct sockaddr *)&serverAddr, sizeof(serverAddr)) < 0)
	{
		perror("vendorAdd: Connect line 30");
		close(sock);
		return 1;
	}
...
// Read request, process and send response 
	...
	if (req.opr == '+')
	{
    		printf("[Vendor +] Processing request for Client PID %d\n", req.clientPid);
	...
	{
    		printf("[Vendor +] Invalid Request for Client PID %d\n", req.clientPid);
	}
