...
int main(int argc, char *argv[])
{
	...
	if(sock == -1)
	{
		perror("vendorDiv: socket line no. 17");
		exit(EXIT_FAILURE);
	}
	...
	printf("vendorDiv Attempting to connect to port %d ..\n",port);
// Connect back to the specific thread spawned this vendor
	if(connect(sock, (struct sockaddr *)&serverAddr, sizeof(serverAddr)) < 0)
	{
		perror("vendorDiv: Connect line 30");
		close(sock);
		return 1;
	}
...
// Read request, process and send response 
	recvExact(sock, &req, sizeof(Request));
	 if(req.opr == '/')
        {
                printf("vendorDIv Processing request for client PID %d\n", req.clientPid);
